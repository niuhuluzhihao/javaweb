...
function checkUsername() {
  // 定义正则
  var username4eg = /^[a-zA-Z0-9]{5,10}$/;
  // 校验用户名
  if (!username4eg.test(loginUser.username)) {
    // 格式不合法
  ...
  <div>
    ...
    <table class="tab" cellspacing="0px">
      <tr class="ltr">
        <td>请输入账号</td>
        <td>
          <input class="ipt" type="text" v-model="loginUser.username"
          @blur="checkUsername()">
      ...
      <tr class="ltr">
        <td>请输入密码</td>
        <td>
          <input class="ipt" type="password" v-model="loginUser.userPwd"
          @blur="checkUserPwd()">
